...
//import static com.mycompany.myapp.MyApplication.theme25;
 ...
 * @author yousra
 */
public class detailEnseignat extends Form {

    Form current;
    String l = "";
    Form previous = Display.getInstance().getCurrent();

    public detailEnseignat(Form previous) {
         ...
         * ****************************************************************************************
         */
        Label a = new Label("Détail Enseignat");
        a.setUIID("Title");
        ...
        Label nom1 = new Label(listeEnseignat.us.getNom());

        Label prenom = new Label("Prénom :");
        Label prenom1 = new Label(listeEnseignat.us.getPrenom());

        Label username = new Label("Nom d'utilisateur:");
        Label username1 = new Label(listeEnseignat.us.getUsername());

        Label email = new Label("Email :");
        Label email1 = new Label(listeEnseignat.us.getEmail());

        Label status = new Label("Status :");
        Switch switchb = new Switch();
        Label status1 = new Label("");

        int enabled = listeEnseignat.us.getEnabled();
        if (enabled == 1) {
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                if (cc) {
                    if (Dialog.show("Activation", "Voulez-vous vraiment activer ce compte ?", "Ok", "Annuler")) {
                        status1.setText("Activer");
                        status1.getAllStyles().setFgColor(0x008000);
                        us.UpdateStatuEnseign(1, listeEnseignat.us.getId());
                        c.revalidate();
                    }
                    ...
                    if (Dialog.show("Désactivation", "Voulez-vous vraiment désactiver ce compte ?", "Ok", "Annuler")) {
                        status1.setText("Désactiver");
                        status1.getAllStyles().setFgColor(0xFF0000);
                        us.UpdateStatuEnseign(0,listeEnseignat.us.getId());
                        c.revalidate();
